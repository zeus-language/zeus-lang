...
namespace ast {
    ...
    struct EnumVariant {
        ...
        EnumVariant(Token name, std::optional<std::shared_ptr<ast::ASTNode> > value) : name(std::move(name)),
            type(EnumVariantType::UNIT), value(std::move(value)) {
        }

        EnumVariant(Token name, EnumVariantType type, std::optional<std::shared_ptr<ast::ASTNode> > value,
                    std::vector<std::shared_ptr<RawType> > associatedRawTypes, std::vector<VariableDeclaration> fields)
        ...
        EnumVariant(const EnumVariant &other) : name(other.name), type(other.type) {
            ...
            this->value = other.value
                              ? std::make_optional<std::shared_ptr<ast::ASTNode> >(other.value.value()->clone())
                              : std::nullopt;
    ...
    public:
        ...
        std::shared_ptr<ASTNode> clone() override {
            ...
            auto node = std::make_shared<EnumDeclaration>(expressionToken(), std::move(variants));
            if (expressionType())
