...
TEST(LexerStringTest, TestUnclosedString) {
    ...
    VerifyTokenPosition(tokens[3], source, 10, 14, "\"Hello, World;");
}
...
let y = 10; // fourth comment
    ...
    VerifyTokenPosition(tokens[5], source, 11, 1, 12, 16, "// first comment");
    EXPECT_EQ(tokens[6].type, Token::Type::LINE_COMMENT);
    VerifyTokenPosition(tokens[6], source, 28, 2, 1, 17, "// second comment");
    EXPECT_EQ(tokens[7].type, Token::Type::LINE_COMMENT);
    VerifyTokenPosition(tokens[7], source, 46, 3, 1, 16, "// third comment");
    EXPECT_EQ(tokens[8].type, Token::Type::KEYWORD);
    VerifyTokenPosition(tokens[8], source, 63, 4, 1, 3, "let");

    EXPECT_EQ(tokens[13].type, Token::Type::LINE_COMMENT);
    VerifyTokenPosition(tokens[13], source, 75, 4, 13, 17, "// fourth comment");
}
...
let y = 10; // fourth comment
    ...
    EXPECT_EQ(tokens[5].type, Token::Type::KEYWORD);
    VerifyTokenPosition(tokens[5], source, 63, 4, 1, 3, "let");
...
TEST(LexerLexNumbersTest, LexHexNumbers) {
    ...
    VerifyTokenPosition(tokens[3], source, 11, 6, "0x1A3F");
    // check other tokens    // Verify all tokens have correct types and positions
    EXPECT_EQ(tokens[0].type, Token::Type::KEYWORD);
    VerifyTokenPosition(tokens[0], source, 0, 3, "let");
    EXPECT_EQ(tokens[1].type, Token::Type::IDENTIFIER);
    VerifyTokenPosition(tokens[1], source, 4, 4, "hex1");
    EXPECT_EQ(tokens[2].type, Token::Type::EQUAL);
    VerifyTokenPosition(tokens[2], source, 9, 1, "=");
    EXPECT_EQ(tokens[4].type, Token::Type::SEMICOLON);
    VerifyTokenPosition(tokens[4], source, 17, 1, ";");
    EXPECT_EQ(tokens[5].type, Token::Type::END_OF_FILE);
}
...
TEST(LexerLexNumbersTest, LexOctalNumbers) {
    ...
    VerifyTokenPosition(tokens[3], source, 11, 7, "0o06670");
}
